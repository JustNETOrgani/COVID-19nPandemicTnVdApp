...
contract coronaVirusTnV{
    ...
    struct approvedHF{
        ...
        address AHF_address; // Address of AHF
    }
    ...
    function updatePersonBlockchainAddr(address personNewAddr, bytes32 HID, bytes32 currenthIPFShash, string memory signatureOnIPFShash) public returns (bool result){
        require (person[HID].HID == HID, 'Invalid Hashed Identifier');
        require (person[HID].hIPFShash == currenthIPFShash, 'Hashed IPFS mismatch');
        require (keccak256(abi.encodePacked(person[HID].signature)) == keccak256(abi.encodePacked(signatureOnIPFShash)), 'Signature mismatch'); // Signature generated using existing signature before new address can be updated.
        // Update person address.
        person[HID].personAddress = personNewAddr;
        return true;
    }
}