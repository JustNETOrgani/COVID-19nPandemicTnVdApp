    ...
    <div class="pageContainer">
        ...
        <el-row>
          <el-col :span="23" :offset="1">
            <el-col :span="7">
              <h3 id="activityHub">AHF-specific Statistics Hub</h3>
            </el-col>
            <el-col :span="5">
            ...
            <el-col :span="7">
              <div class="rowAlignment">
                  <el-form
                      :model="ahfspecificTasks"
                      :rules="rules"
                      ref="ahfspecificTasks"
                      label-width="120px"
                  >
                      <el-form-item label="Required task" prop="deployerTask">
                          <el-select
                              v-model="ahfspecificTasks.ahfTasks"
                              style="width:100%"
            ...
            <div class="rowAlignment">
            <el-col :span="5">
              <el-button type="info" round :loading="getDataBtnLoadState" @click="submitForm('ahfspecificTasks')">Get data</el-button>
            </el-col>
            </div>
          </el-col>
        </el-row>
  ...
  created () {
    if (!ethEnabled()) {
      ...
      this.getAccount().then(accounts => {
        this.addressOfAHF = accounts[0]
        console.log('Current account: ', this.addressOfAHF)
      })
    }
  },
  watch: {
  ...
  methods: {
    ...
    switchAccount () {
      ...
      window.ethereum.on('accountsChanged', function (accounts) {
        myRoot.addressOfAHF = accounts[0]
        console.log('Selected account: ', myRoot.addressOfAHF)
        myRoot.$message({
    ...
    submitForm (formName) {
      this.$refs[formName].validate(valid => {
        ...
        if (valid) {
          ...
          if (this.ahfspecificTasks.ahfTasks === 'allTestsNvacResults') {
            this.getAllTestsAndVaccResults()
          } else if (this.ahfspecificTasks.ahfTasks === 'updatedTestsNvacResults') {
    ...
    getAllTestsAndVaccResults () {
      ...
      blockCovid.getPastEvents('onboarded', { filter: { txInitiator: [this.addressOfAHF] }, fromBlock: 0, toBlock: 'latest' },
        (err, results) => {
          if (err) {
          ...
          // Get the data and display.
            if (Object.keys(results).length === 0) {
              ...
              this.$message({
                message: 'Sorry! This AHF has no person onboarded on BlockCovid.',
                type: 'info'
              })
            } else {
    ...
    getupdatedTestsNvacRes () {
      ...
      var blockCovid = new web3.eth.Contract(ABI, contractAddress, { defaultGas: suppliedGas })// End of ABi Code from Remix.
      console.log('Contract instance for access type retrieval created.')
      blockCovid.getPastEvents('personUpdated', { filter: { txInitiator: [this.addressOfAHF] }, fromBlock: 0, toBlock: 'latest' },
        (err, results) => {
          if (err) {
            this.updatedTnVData = false
          ...
          // Get the data and display.
            if (Object.keys(results).length === 0) {
              ...
              this.defaultPageItem = true
              this.$message({
                message: 'Sorry! This AHF has no person updated record on BlockCovid.',
                type: 'info'
              })
            } else {
              // Results is not empty hence get and display.
